...
struct BadgeDemoView: View {
    ...
    private func renderBadge() async {
        ...
        case .complex:
            guard let complexData = loadBundledBadgeData() else {
                errorMessage = "Missing bundled resource: badge.svg"
                renderedImage = nil
                return
            }
            data = complexData
    ...
    private var sourceDescription: String {
        ...
        case .complex:
            return "Source: bundled badge.svg (from app resources)"
        }
    }

    private func loadBundledBadgeData() -> Data? {
        if let url = Bundle.main.url(forResource: "badge", withExtension: "svg"),
           let data = try? Data(contentsOf: url) {
            return data
        }
        if let url = Bundle.main.resourceURL?.appendingPathComponent("badge.svg"),
           let data = try? Data(contentsOf: url) {
            return data
        }
        return nil
